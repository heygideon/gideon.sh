<script lang="ts">
	...
	const { id, children }: WindowProps = $props();
...
>
	<div
		...
		class={[
			'flex size-full max-h-max min-h-0 flex-1 flex-col overflow-clip rounded-t-lg border-t border-stone-300 bg-white shadow-lg transition',
			openWindow.minimised ? ' translate-y-12 opacity-0' : 'pointer-events-auto'
		]}
	>
		<div
